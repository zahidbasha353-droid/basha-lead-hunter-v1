...
if st.button("🚀 Start Vettai"):
    ...
    finally: driver.quit()import streamlit as st
import time
import pandas as pd
import re
import requests
...
with col_head2:
    st.metric(label="💰 Wallet Balance", value=f"₹{user_data['credits']}")
...
if role == "client":
    with st.sidebar.expander("💎 Wallet / Recharge", expanded=True):
        st.write(f"**Current Balance: ₹{user_data['credits']}**")
...
# 2. Check Balance
current_balance = user_data['credits']
